...
from auto_dr.rnd_network import RNDNetwork
...
delta = 0.1 
t_L, t_H = 50, 100   # Adjust these thresholds as needed
m = 10

print(f'Start AutoDR Training: {datetime.now()}')
writer = SummaryWriter('auto_dr/intrinsic_reward/')
...
with open('reward_and_parameters_RND1.txt', 'w') as f:
    for episode in range(10000):
        ...
        writer.add_scalar(f'Episode Intrinsic Reward', intrinsic_reward, episode)
        writer.add_scalar(f'Episode Performance', reward, episode)
        writer.add_scalar(f'Episode Entropy', entropy, episode)
        writer.add_scalar(f'Episode Average Speed', speed, episode)
        writer.add_scalar(f'Episode Average Stability', stability, episode)
        writer.add_scalar(f'Episode Average Balance', balance, episode)
